...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune E2E Transformer on target-subject calibration trials.")
    ...
    base_dir = Path(args.output_root) / f"base_{args.target_subject}"
    base_model_path = base_dir / f"e2e_transformer_base_{args.target_subject}.pth"

    out_dir = Path(args.output_root) / f"finetuned_{args.target_subject}"
    out_dir.mkdir(parents=True, exist_ok=True)

    if not base_model_path.exists():
        raise FileNotFoundError(f"Missing base model: {base_model_path}")

    print("E2E fine-tuning")
    ...
    print("calibration windows:", len(dataset))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        pbar = tqdm(loader, desc=f"Fine-tune epoch {epoch}/{args.epochs}", unit="batch")
        ...
        pd.DataFrame(rows).to_csv(out_dir / "fine_tuning_log.csv", index=False)
        print(f"Fine-tune epoch {epoch}: loss={epoch_loss:.4f}, accuracy={epoch_acc:.4f}")

    model_path = out_dir / f"e2e_transformer_finetuned_{args.target_subject}.pth"
    ...
    metadata = {
        ...
        "pretrained_cnn": not args.no_pretrained_cnn,
        "num_calibration_windows": len(dataset),
